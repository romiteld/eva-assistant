...
CREATE TABLE IF NOT EXISTS deal_creation_metrics (
  ...
  INDEX idx_deal_metrics_user_id (user_id),
  INDEX idx_deal_metrics_created_at (created_at),
  INDEX idx_deal_metrics_source (source),
  INDEX idx_deal_metrics_duration (duration_ms)
);
...
COMMENT ON COLUMN deal_creation_metrics.duration_ms IS 'Total time in milliseconds to create the deal';
COMMENT ON COLUMN deal_creation_metrics.steps IS 'JSON array of individual step timings and results';