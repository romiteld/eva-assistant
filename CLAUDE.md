...
- Current System Overview:
  ...
  Frontend (Vercel)
      ↓
  OAuth 2.0 PKCE → Microsoft Entra ID (SPA Authentication)
      ↓
  ...
  Note: Using Single-Page Application (SPA) configuration in Entra ID, which requires client-side token exchange only.
...
## Talk to Eva Multimodal Voice Assistant

### Implementation Overview

- Eva Brain Service (/lib/services/eva-brain.ts)
  - Integrates Gemini Pro 2.5 and Flash 2.5 models
...
- Whisper Transcription Service (/lib/services/whisper-transcription.ts)
  - OpenAI Whisper API integration
  - Real-time and batch transcription
  - Audio recording with WebRTC
  - Streaming support for continuous transcription

- Eva Agent Integration (/lib/agents/eva-agent.ts)
  - Registered as A2A agent for cross-agent communication
  - Voice command processing
  - Tool execution with result tracking
  - Conversation storage in Supabase

- Talk to Eva Page (/app/dashboard/talk-to-eva/page.tsx)
  - Beautiful voice interface with:
...
- Voice session tracking with metrics
...
- Voice Session Hook (/hooks/useVoiceSession.ts)
  - Automatic session creation
  - Real-time metrics tracking
  - Command and tool usage logging
  - Session analytics

### Key Features

- ✅ Multimodal AI with Gemini Pro/Flash
- ✅ Voice input with OpenAI Whisper
- ✅ Voice output with ElevenLabs TTS
- ✅ Web search with Firecrawl API
- ✅ Dashboard navigation and control
- ✅ A2A agent workflow execution
- ✅ Complete session tracking and analytics
- ✅ Real-time voice visualization
- ✅ File upload support (images and documents)
- ✅ Drag-and-drop file attachments
- ✅ Multimodal processing with Gemini Vision
...
- System can now:
  ...
  4. Execute tools (search, navigate, query data)
  5. Respond with natural voice
  6. Track all interactions in the database
  7. Process uploaded files (images with vision AI, documents with text extraction)
  8. Handle multimodal conversations with context-aware responses

### File Upload Implementation

- **File Upload Service** (`/lib/services/chat-file-upload.ts`)
  - Extends base file upload service for chat-specific features
  - Handles images (PNG, JPG, WEBP) and documents (PDF, TXT, MD)
  - 10MB file size limit
  - Automatic base64 encoding for images
  - Text extraction for documents

- **Storage Configuration**
  - Supabase Storage bucket: `chat-uploads`
  - Files organized by: `user_id/session_id/filename`
  - Database table: `chat_uploads` with RLS policies
  - Automatic cleanup on file deletion

- **UI Components**
  - FileUploadZone component with drag-and-drop support
...
- **Eva Brain Updates**
  - ProcessInput method accepts attachments array
  - Multimodal content prepared for Gemini API
  - Images sent as inline data with base64 encoding
  - Documents included as text in the prompt
