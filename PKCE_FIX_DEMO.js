...
function mockSetCookie(name, value) {
...
function mockSetItem(storage, key, value) {
  if (storage === 'session') {
...
function mockGetItem(storage, key) {
  if (storage === 'session') {
...
function demonstrateEnhancedPKCERetrieval(
  ...
  if (!codeVerifier) {
    try {
      const stateData = JSON.parse(Buffer.from(expectedState, 'base64').toString());
      if (stateData.pkce) {
